...
- [x] Post-game review module (draft-decision scope, not match film) — done 18 Aug 2026: `/match/[id]` single-game draft breakdown, plus a rolling-HHI trend chart and pick/ban win-rate-delta tables on `/team/[slug]`. No pipeline changes needed. Spec: `docs/superpowers/specs/2026-08-18-post-game-review-design.md`. Plan: `docs/superpowers/plans/2026-08-18-post-game-review.md`. `/match/[id]` later replaced by `/series/[seriesId]` in the 20 Aug 2026 Match Log redesign (current-context.md) — same draft-breakdown content, grouped by series instead of one route per game.
- [x] Season 17 vs Season 18 trend views — done 18 Aug 2026: `presenceDelta` reuses the existing season-filterable `presence`/`hhi`, no pipeline changes. New sections on `/team/[slug]` and `/league` compare each hero's S17 rate to S18-to-date, game counts shown next to every number instead of hidden. Spec: `docs/superpowers/specs/2026-08-18-season-trend-design.md`. Plan: `docs/superpowers/plans/2026-08-18-season-trend.md`.
- [x] Per-Role & Flex Scouting — done 27 Aug 2026: role filter on Team Scouting & League Meta Draft, lane-specific HHI callouts, team Flex Picks card with `RoleDistributionBar`, plus a dedicated `/roles` dashboard with Team Predictability Matrix and Tournament Flex Picks. Spec: `docs/superpowers/specs/2026-08-27-per-role-flex-scouting-design.md`. Plan: `docs/superpowers/plans/2026-08-27-per-role-flex-scouting.md`.
- [x] Head-to-Head Matchup Tool — done 27 Aug 2026: dedicated `/matchup` route with dual-team selectors, "⇄ Swap" controls, season filtering, direct series & game records, Blue vs Red side performance, 3-way Draft Clash priority tabs (Contested, Team 1 Signatures, Team 2 Signatures), lane-by-lane predictability & comfort picks, and direct series draft accordions. Spec: `docs/superpowers/specs/2026-08-27-head-to-head-matchup-design.md`. Plan: `docs/superpowers/plans/2026-08-27-head-to-head-matchup.md`.
- [x] Side Priority Analysis (Blue vs Red) — done 27 Aug 2026: dedicated `/sides` route with tournament Blue vs Red win rate split banner, 8-team side asymmetry matrix, and 3-way side-specific hero priority rankings (First-Pick Priority, Counter-Pick Priority, Side Win-Rate Swings). Spec: `docs/superpowers/specs/2026-08-27-side-priority-analysis-design.md`. Plan: `docs/superpowers/plans/2026-08-27-side-priority-analysis.md`.
- [x] Interactive Draft Sandbox — done 27 Aug 2026: dedicated `/sandbox` route with 20-step official tournament draft simulator, Dual Coach and Solo vs AI modes, real-time scouting recommendations, live 5-lane role coverage checklist, and draft HHI scoring. Spec: `docs/superpowers/specs/2026-08-27-interactive-draft-sandbox-design.md`. Plan: `docs/superpowers/plans/2026-08-27-interactive-draft-sandbox.md`.
- [x] Re-check whether Liquipedia has migrated MLBB to the new `match2` system — checked 18 Aug 2026, still not migrated (0 `MatchGroup`/`match2` occurrences, live fetch and committed snapshot both agree). Parser stays necessary. See data-source.md.
- [x] Revisit distribution: is anyone in the scene actually using v1? — checked 18 Aug 2026: no, nobody yet. Confirms planning.md's "biggest open assumption" is still open. Before building more Next/Later features, find out why (visibility? trust? wrong audience?) — see planning.md.
